...
# Log temperature and fan speed to log file
log_to_file() {
    local temp=$1
    local fan_speed=$2
    echo "$(date): Temp: $temp°C, Fan Speed: $fan_speed RPM" >> $LOGFILE
}
...
if [ "$TEST_MODE" = true ]; then
    ...
    echo "Monitoring temperature and fan speed every 15 seconds for 2 minutes..."
    for i in {1..8}; do
        TEMP=$(get_temperature)
        FAN_SPEED=$(get_fan_speed)
        echo "[$(date '+%H:%M:%S')] Current Temp: $TEMP°C, Fan Speed: $FAN_SPEED RPM"
        log_to_file $TEMP $FAN_SPEED
        sleep 15
    done
    echo "Temperature and fan speed monitoring complete."
else
...
trigger_api_call() {
    if [ "$TEST_MODE" = false ]; then
        ...
        echo "API call would have been triggered: Temp-now=$TEMP, Temp-last=$LAST_TEMP, Fan-speed=$FAN_SPEED"
    fi
}
...
# Display the last 5 logged temperatures and fan speeds
echo "Last 5 temperatures and fan speeds logged:"
tail -n 5 $LOGFILE
